@echo off
setlocal enabledelayedexpansion

echo 🚀 InfinityFire Heroku Deployment Script
echo =========================================

REM Check if Heroku CLI is installed
heroku --version >nul 2>&1
if %errorlevel% neq 0 (
    echo ❌ Heroku CLI is not installed.
    echo Please install it first from: https://devcenter.heroku.com/articles/heroku-cli
    pause
    exit /b 1
)

for /f "tokens=*" %%i in ('heroku --version') do set HEROKU_VERSION=%%i
echo ✅ Heroku CLI detected: !HEROKU_VERSION!

REM Check if user is logged in
heroku auth:whoami >nul 2>&1
if %errorlevel% neq 0 (
    echo 🔐 Please log in to Heroku...
    heroku login
)

for /f "tokens=*" %%i in ('heroku auth:whoami') do set HEROKU_USER=%%i
echo ✅ Logged in as: !HEROKU_USER!

REM Get app name
set /p APP_NAME="Enter your Heroku app name (or press Enter to let Heroku generate one): "

if "!APP_NAME!"=="" (
    echo 🆕 Creating Heroku app with auto-generated name...
    for /f "tokens=*" %%i in ('heroku create --json') do set CREATE_OUTPUT=%%i
    REM Extract app name from JSON output (simplified)
    set APP_NAME=infinityfire-!RANDOM!
    echo ✅ App created: !APP_NAME!
) else (
    echo 🆕 Creating Heroku app: !APP_NAME!
    heroku create "!APP_NAME!"
)

echo.
echo 🗄️ Adding PostgreSQL database...
heroku addons:create heroku-postgresql:mini --app "!APP_NAME!"

echo.
echo 🔑 Setting up environment variables...

REM Generate JWT secrets (Windows doesn't have openssl by default)
set JWT_SECRET=infinityfire-jwt-secret-!RANDOM!!RANDOM!!RANDOM!
set JWT_REFRESH_SECRET=infinityfire-refresh-secret-!RANDOM!!RANDOM!!RANDOM!

heroku config:set JWT_SECRET="!JWT_SECRET!" --app "!APP_NAME!"
heroku config:set JWT_REFRESH_SECRET="!JWT_REFRESH_SECRET!" --app "!APP_NAME!"
heroku config:set NODE_ENV=production --app "!APP_NAME!"
heroku config:set PORT=5000 --app "!APP_NAME!"

echo.
echo ⚠️  IMPORTANT: You need to set your AWS S3 credentials manually:
echo    heroku config:set AWS_ACCESS_KEY_ID=your_key --app !APP_NAME!
echo    heroku config:set AWS_SECRET_ACCESS_KEY=your_secret --app !APP_NAME!
echo    heroku config:set AWS_REGION=your_region --app !APP_NAME!
echo    heroku config:set AWS_S3_BUCKET=your_bucket --app !APP_NAME!

echo.
set /p AWS_SET="Have you set your AWS credentials? (y/n): "

if /i not "!AWS_SET!"=="y" (
    echo ❌ Please set your AWS credentials first, then run this script again.
    pause
    exit /b 1
)

echo.
echo 🚀 Deploying to Heroku...

REM Add Heroku remote if not already added
git remote | findstr heroku >nul
if %errorlevel% neq 0 (
    heroku git:remote -a "!APP_NAME!"
)

REM Push to Heroku
git push heroku main

echo.
echo 🗃️ Setting up database...
heroku run npm run db:setup --app "!APP_NAME!"

echo.
echo 🌐 Opening your app...
heroku open --app "!APP_NAME!"

echo.
echo 🎉 Deployment completed successfully!
echo.
echo 📊 Useful commands:
echo    View logs: heroku logs --tail --app !APP_NAME!
echo    Check status: heroku ps --app !APP_NAME!
echo    View config: heroku config --app !APP_NAME!
echo    Restart app: heroku restart --app !APP_NAME!
echo.
echo 🔗 Your app is available at: https://!APP_NAME!.herokuapp.com
echo.
echo 📚 For more information, see HEROKU_DEPLOYMENT.md
pause 